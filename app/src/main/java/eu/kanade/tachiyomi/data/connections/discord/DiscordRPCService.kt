...
class DiscordRPCService : Service() {
    ...
    companion object {
        ...
        internal suspend fun setScreen(
            ...
            val state = readerData.chapterNumber ?: context.resources.getString(discordScreen.state)

            val imageUrl = readerData.thumbnailUrl ?: discordScreen.imageUrl

            rpc!!.updateRPC(
                Log.d("discord_rpc_tachiyomisy", "UpdateRPC : $name, $details, $state, ${readerData.thumbnailUrl}, $since"),
                activity = Activity(
                    name = name,
                    details = details,
                    state = state,
        ...
        internal suspend fun setReaderActivity(context: Context, readerData: ReaderData = ReaderData()) {
            ...
            Log.d("discord_rpc_tachiyomisy", "SetReaderActivity : $mangaTitle, ${readerData.chapterNumber}, ${readerData.chapterProgress}")
            ...
            withIOContext {
                ...
                val response = if (!discordIncognito) {
                    try {
                        Log.d("discord_rpc_tachiyomisy", "SetReaderActivity : getting: https://kizzy-api.vercel.app/image?url=${readerData.thumbnailUrl}")
                        client.newCall(
                ...
                Log.d("discord_rpc_tachiyomisy", "SetReaderActivity : response: ${response?.body?.string()}")
                ...
                Log.d("discord_rpc_tachiyomisy", "SetReaderActivity : thumb: $mangaThumbnail")
